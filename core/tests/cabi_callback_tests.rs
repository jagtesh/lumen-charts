...
extern "C" fn test_event_callback(
    param: *const lumen_charts_core::ChartEventParam,
...
fn test_range_callback_abi_compat() {
    ...
    let cb: lumen_charts_core::RangeChangeCallback = test_range_callback;
    cb(100.0, 200.0, ud);
...
fn test_size_callback_abi_compat() {
    ...
    let cb: lumen_charts_core::SizeChangeCallback = test_size_callback;
    cb(800.0, 600.0, ud);
    assert_eq!(counter.load(Ordering::SeqCst), 1);
}
...
fn test_chart_event_callback_abi_compat() {
    ...
    let param = lumen_charts_core::ChartEventParam {
        time: 1704153600,
        logical: 42.0,
        point_x: 100.0,
        point_y: 200.0,
    ...
    let cb: lumen_charts_core::ChartEventCallback = test_event_callback;
...
fn test_chart_event_param_layout() {
    ...
    assert_eq!(
        std::mem::size_of::<lumen_charts_core::ChartEventParam>(),
...
fn test_markers_json_roundtrip() {
    use lumen_charts_core::overlays::{MarkerPosition, MarkerShape, SeriesMarker};

    let markers = vec![
        SeriesMarker::new(1000, MarkerShape::ArrowUp, MarkerPosition::BelowBar)
            .with_color(lumen_charts_core::draw_backend::Color([0.0, 1.0, 0.0, 1.0]))
            .with_text("Buy")
            .with_size(12.0),
        SeriesMarker::new(2000, MarkerShape::ArrowDown, MarkerPosition::AboveBar)
            .with_color(lumen_charts_core::draw_backend::Color([1.0, 0.0, 0.0, 1.0]))
            .with_text("Sell")
            .with_size(10.0),
    ];
    ...
    let mut overlays = lumen_charts_core::overlays::Overlays::new();
